...
const MainPage: React.FC = () => {
  ...
  const mainSubPage = useSelector((state: State) => state.mainSubPage);

  const mainStyle = context.qrScanning ? styles.hidden : styles.visible;

  const pageMapping: Record<string, JSX.Element> = {
    mainPage: (
      ...
      <View style={mainStyle}>
        <View style={styles.background}>
          <View style={styles.body}>
            {currentPage ? pageMapping[currentPage] : null}
            <Send />
            <LowerBar />
          </View>
        </View>
